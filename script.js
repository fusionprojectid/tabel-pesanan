...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const elements = {
        ...
        cetakBtn: document.getElementById('cetak'),
    ...
    const renderHasil = (searchTerm = '') => {
        ...
        if (filteredPesanan.length === 0) {
            elements.tabelHasil.innerHTML = `<tr><td colspan="8" class="text-center p-4 text-gray-500">Belum ada data pesanan.</td></tr>`;
        ...
        for (const barang in rekapBarang) {
            ...
            card.className = 'bg-white p-4 rounded-lg shadow-md border text-center';
            card.innerHTML = `
                <p class="text-sm text-gray-600">Total ${barang}</p>
                <p class="text-3xl font-bold" style="color: var(--brand-dark);">${rekapBarang[barang]}</p>
            `;
        ...
        filteredPesanan.forEach(p => {
            if (!groupedPesanan[p.nama]) {
                groupedPesanan[p.nama] = [];
            }
        ...
        for (const nama in groupedPesanan) {
            ...
            items.forEach((item, itemIndex) => {
                ...
                if (itemIndex === 0) {
                    tableRowsHTML += `
                        <tr class="border-b text-center hover:bg-orange-50">
                            <td class="p-3 align-top" rowspan="${rowSpanCount}">${groupIndex}</td>
                            <td class="p-3 text-left align-top font-semibold" rowspan="${rowSpanCount}">${nama}</td>
                            <td class="p-3 text-left">${item.barang}</td>
                            <td class="p-3 text-left">${item.detail || '-'}</td>
                            <td class="p-3">${item.jumlah}</td>
                            <td class="p-3 money text-right">${formatRupiah(item.hargaSatuan)}</td>
                            <td class="p-3 money text-right">${formatRupiah(subtotal)}</td>
                            <td class="p-3 flex justify-center items-center gap-2">${actionButtons}</td>
                        </tr>
                    `;
                } else {
                    tableRowsHTML += `
            ...
            tableRowsHTML += `
                <tr class="bg-gray-100 font-bold border-b-4 border-white">
                    <td colspan="6" class="p-2 text-right">Total untuk ${nama}</td>
                    <td class="p-2 money text-right">${formatRupiah(totalPerNama)}</td>
                    <td class="p-2 text-center">
                        <button class="delete-group-btn text-xs text-white bg-red-500 hover:bg-red-700 px-2 py-1 rounded" data-nama="${nama}">Hapus Semua</button>
                    </td>
                </tr>
            `;
    ...
    const tambahPesanan = async (e) => {
        ...
        if (selectedBarang === 'Kaos Polo' && elements.bahanPoloSelect.value) {
            detailArray.push(elements.bahanPoloSelect.value);
        }

        if (selectedBarang === 'Kaos Polo') {
            detailArray.push('Panjang');
        } else if (elements.opsiTipeLengan.style.display !== 'none' && elements.tipeLenganSelect.value) {
            detailArray.push(elements.tipeLenganSelect.value);
        }
        ...
        const hargaSatuan = calculatePrice();
        const newItem = {
            id: Date.now(),
    ...
    const deleteItem = async (id) => {
        ...
        if (confirmed) {
            ...
            pesanan = pesanan.filter(item => item.id !== id);
            saveDraft(pesanan);
    ...
    const deleteGroup = async (nama) => {
        ...
        if (confirmed) {
            let pesanan = loadDraft();
            pesanan = pesanan.filter(item => item.nama !== nama);
            saveDraft(pesanan);
            renderHasil(elements.searchRekap.value);
        }
    };
    ...
    const generateEmailBody = () => {
        ...
        pesanan.forEach((p, index) => {
            ...
            body += `Nama    : ${p.nama}\n`;
            body += `Barang  : ${p.barang}\n`;
            body += `Detail  : ${p.detail || '-'}\n`;
            body += `Jumlah  : ${p.jumlah} pcs\n`;
            body += `Harga   : ${formatRupiah(p.hargaSatuan)}\n`;
            body += `Subtotal: ${formatRupiah(p.jumlah * p.hargaSatuan)}\n`;
            body += "------------------------------------------------\n";
        });
        const grandTotal = pesanan.reduce((total, p) => total + (p.hargaSatuan * p.jumlah), 0);
    ...
    const exportToExcel = async () => {
        ...
        const dataForExport = pesanan.map((p, index) => ({
            'No.': index + 1,
            'Nama Pemesan': p.nama,
            'Barang': p.barang,
            'Detail': p.detail,
            ...
            'Subtotal': p.jumlah * p.hargaSatuan
        }));
        const grandTotal = pesanan.reduce((total, p) => total + p.hargaSatuan * p.jumlah, 0);
        ...
        XLSX.writeFile(workbook, `Rekap Pesanan_${tgl}.xlsx`);
    };
    ...
    elements.cetakBtn.addEventListener('click', () => window.print());
